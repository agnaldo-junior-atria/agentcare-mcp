...
import { logToFile } from "../../../helpers/logs.js";
...
export class FhirClient {
  ...
  constructor(baseUrl: string) {
    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        // Handle error responses here (e.g., log error details)
        if (error.response) {
  ...
  async executeQuery(queryParams: any) {
    ...
    const response = await this.client.get(
      `/${queryParams.resourceType}?${params}`
    );
  ...
  async findPatient(args: any) {
    ...
    if (!response.data?.entry?.[0]?.resource) {
      return this.formatResponse("fhir://Patient/search", {
  ...
  handleError(error: any) {
    const errorMessage = axios.isAxiosError(error)
      ? `FHIR API error: ${
          error.response?.data?.issue?.[0]?.details?.text ?? error.message
        }`
      : error.message;
  ...
  async getPatientObservations(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/observations`,
      response.data
    );
  }
  ...
  async getPatientConditions(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/conditions`,
      response.data
    );
  }
  ...
  async getPatientMedications(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/medications`,
      response.data
    );
  }
  ...
  async getPatientEncounters(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/encounters`,
      response.data
    );
  }
  ...
  async getPatientAllergies(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/allergies`,
      response.data
    );
  }
  ...
  async getPatientProcedures(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/procedures`,
      response.data
    );
  }
  ...
  async getPatientCareTeam(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/careteam`,
      response.data
    );
  }
  ...
  async getPatientCarePlans(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/careplans`,
      response.data
    );
  }
  ...
  async getPatientVitalSigns(args: any) {
    ...
    const response = await this.client.get(`/Observation?${params}`);
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/vital-signs`,
      response.data
    );
  }
  ...
  async getMedicationHistory(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/medication-history`,
      response.data
    );
  }
  ...
  async getPatientLabResults(args: any) {
    ...
    const response = await this.client.get(`/Observation?${params}`);
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/lab-results`,
      response.data
    );
  }
  async getPatientAppointments(args: any) {
    ...
    return this.formatResponse(
      `fhir://Patient/${args.patientId}/appointments`,
      response.data
    );
  }
  ...
  getRelevantMetrics(observations: any[], condition: any): string[] {
    ...
    return observations
      ...
      .map((obs: any) => {
        ...
        const date =
          obs.resource.effectiveDateTime?.split("T")[0] || "unknown date";
        const name =
          obs.resource.code?.coding?.[0]?.display || "Unknown metric";
