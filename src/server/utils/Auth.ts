...
export class Auth {
  ...
  private isTokenExpired(): boolean {
    ...
    // Add 5 minute buffer
    return (
      new Date(this.token.expires_at).getTime() - 5 * 60 * 1000 < Date.now()
    );
  }
    ...
    app.get("/oauth/callback", async (req, res) => {
      ...
      try {
        if (error) {
          ...
          const token: Token = await this.exchangeCodeForToken(
            code as string,
            this.authConfig.authorizationMethod
          );
          this.token = token;
      ...
      try {
        const filePath = fileURLToPath(
          new URL("./auth-success.html", import.meta.url)
        );
